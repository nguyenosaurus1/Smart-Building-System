...
def print_msg(client, userdata, message):
    ...
    # print(type(data))
    light_threshold = db.execute("SELECT light_threshold FROM room WHERE room_id="+data["room"]).fetchone()[0]
    if int(data["value"]) > light_threshold:
        devices = db.execute("SELECT device_id FROM device WHERE type = 'light' AND room_id="+data["room"])
        ...
        try:
            ...
            db.execute("UPDATE device SET status=0 WHERE type='light' AND room_id="+ data["room"])
            db.commit()
        finally:
            db.close()
        print("Done")
